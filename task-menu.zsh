...
# Fuzzy Taskfile picker for tm. The menu is written to stderr so the selected
# task command can be placed in the calling Zsh prompt without running it.
...
render_menu() {
  ...
  if (( ${#task_names} == 0 )); then
    printf '\033[2mNo tasks match "%s".\033[0m\n' "$query" >&2
